...
def create_price_per_lot_size(df): # creating a new column for price per lot size
    df["price_per_lot_size"] = df["price"] / df["lot_size"]
    df["price_per_lot_size"] = df["price_per_lot_size"].round(2) # round to 2 decimal places
    return df
